...
let pomodorosDia    = parseInt(localStorage.getItem("pomodorosDia") || "0");
let minutosDia      = parseInt(localStorage.getItem("minutosDia") || "0");
...
if (ultimoDia !== hoje) {
    pomodorosDia = 0;
    minutosDia = 0;
    localStorage.setItem("pomodorosDia", "0");
    localStorage.setItem("minutosDia", "0");
...
function entrarEmPausa() {
    ...
    pomodorosDia++;
    localStorage.setItem("pomodorosDia", pomodorosDia);
    updateMetas();
    marcarDiaEstudado();
    renderCalendario();
...
function faseTerminou() {
    ...
    if (!emPausa) {
        ...
        localStorage.setItem("minutosDia", minutosDia);
        segundosEstudados = 0;
        entrarEmPausa();
        rodando = true;
        atualizarBotaoPlay();
...
function marcarDiaEstudado() {
    const dias = JSON.parse(localStorage.getItem("diasEstudados") || "[]");
    if (!dias.includes(hoje)) {
        dias.push(hoje);
        localStorage.setItem("diasEstudados", JSON.stringify(dias));
    }
}
